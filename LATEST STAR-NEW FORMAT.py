...
def is_engine_chassis_green(D):
    """Check if Engine Number and Chassis Number cells are already green (row already processed)."""
    try:
        excel_row = D.get("_excel_row", 4)
        wb = openpyxl.load_workbook(EXCEL_PATH)
        ws = wb["Details"]
        fill16 = ws.cell(row=excel_row, column=16).fill
        fill17 = ws.cell(row=excel_row, column=17).fill
        green = "92D050"
        def is_green(fill):
            return (fill and fill.fill_type == "solid" and
                    fill.fgColor and str(fill.fgColor.rgb).upper().endswith(green))
        return is_green(fill16) and is_green(fill17)
    except Exception:
        return False
...
async def run(D, skip_step1=False):
    steps       = []
    proposal_no = ""
...
if __name__ == "__main__":
  ...
  ──────────────────────────────────────────────────────────────
    ...
    while True:
        ...
        next_row = asyncio.run(run(D, skip_step1=(row_count > 1)))
